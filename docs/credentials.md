...
| `PF_SCOPES` | OAuth `/authorize` request | Code constant (overridable per practice via env) | Default: `user/Patient.read openid fhirUser offline_access`. Adjust per your EHR's supported scopes. |
...
|---|---|---|
| AWS account `<YOUR_AWS_ACCOUNT_ID>` | Development + pilot account | — |
...
For local development, credentials live in `.env` at the repo root. `.env` is gitignored. `.env.example` documents the shape and is committed.
...
Production replaces this with Secrets Manager fetches at Lambda init.
